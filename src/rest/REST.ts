...
export class REST {
	...
	private generateRequest(path: string, opts: Partial<APIRequest> = {}): APIRequest {
	...
	delete<
		...
		Route extends Extract<DeleteRoutes, { path: Path; parts: Count<Path, '/'>; method: 'DELETE' }>
	>(path: Path, options: Partial<APIRequest> = {}): Promise<Route['response']> {
	...
	put<
		...
		Route extends Extract<PutRoutes, { path: Path; parts: Count<Path, '/'>; method: 'PUT' }>
	>(path: Path, options: Partial<APIRequest> = {}): Promise<Route['response']> {
